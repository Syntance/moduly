...
import { OgImageField } from "@/components/seo/og-image-field";
import type { SeoMeta, SiteSettings } from "@/lib/seo-data";
...
export function SeoForm(props: Props) {
  ...
  return <PageSeoForm path={props.path} initial={props.initial} />;
}
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-2xl flex-col gap-5">
      <div className="flex flex-col gap-1.5">
        ...
        <Input value={title} onChange={(e) => { setTitle(e.target.value); setSaved(false); }} required className="h-10" />
      </div>
      <div className="flex flex-col gap-1.5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Input
          value={titleTemplate}
          onChange={(e) => { setTitleTemplate(e.target.value); setSaved(false); }}
          placeholder="%s | Outdoor Store"
          className="h-10"
        />
        <p className="text-xs text-muted-foreground">
          <code className="text-[11px]">%s</code> to tytuł danej podstrony — reszta szablonu dokleja się automatycznie.
          Przykład: przy szablonie <code className="text-[11px]">%s | Outdoor Store</code> strona „Sklep” ma tytuł{" "}
          <strong className="font-medium text-foreground">Sklep | Outdoor Store</strong> w zakładce i w Google.
        </p>
      </div>
      <div className="flex flex-col gap-1.5">
        ...
        <Input
          ...
          onChange={(e) => { setGoogleVerification(e.target.value); setSaved(false); }}
          className="h-10"
        />
      </div>
      <OgImageField
        ...
        value={defaultOg}
        onChange={(url) => { setDefaultOg(url); setSaved(false); }}
      ...
      <FormFooter saved={saved} pending={pending} />
    </form>
  );
}

function PageSeoForm({ path, initial }: { path: string; initial: SeoMeta | undefined }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-2xl flex-col gap-5">
      <p className="text-sm text-muted-foreground">
        SEO dla podstrony <span className="font-mono text-xs">{path}</span>. Puste pola korzystają z domyślnych wartości witryny.
      </p>
      <SeoFields seo={seo} onChange={updateSeo} />
      <FormFooter saved={saved} pending={pending} />
    </form>
...
function FormFooter({ saved, pending }: { saved: boolean; pending: boolean }) {
  return (
    <div className="flex flex-col gap-2">
      {saved ? (
        <p role="status" className="text-sm text-emerald-700">
          Zapisano. Zmiany widoczne po odświeżeniu strony sklepu (demo).
        </p>
      ) : null}
